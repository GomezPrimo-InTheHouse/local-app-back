 ...
 */
export const registrarAbono = async (req, res) => {
  const { orden_trabajo_id, monto, metodo_pago, observaciones } = req.body;

  if (!orden_trabajo_id || !monto || !metodo_pago) {
    return res.status(400).json({
      error: 'Faltan campos obligatorios: orden_trabajo_id, monto, metodo_pago.'
    });
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('id, orden_trabajo_id, monto, fecha_pago, metodo_pago')
      .single();

    if (error) {
      console.error('Error Supabase al registrar abono:', error);
      return res.status(500).json({
    ...
    return res.status(201).json({ message: 'Abono registrado exitosamente.', pago: data });
  } catch (err) {
 ...
 * Historial de pagos del cliente via RPC.
 */
export const obtenerHistorialCliente = async (req, res) => {
  const clienteIdInt = parseInt(req.params.clienteId, 10);
  ...
  try {
    ...
    if (error) {
      console.error('Error RPC get_historial_pagos_cliente:', error);
      if (error.code === '42883') {
        return res.status(500).json({
          error: 'Error de configuración en BD',
          details: 'La función get_historial_pagos_cliente no existe o no tiene permisos.',
        });
      }
      return res.status(500).json({
        error: 'Error al obtener historial del cliente.',
    ...
    console.error('Error interno obtenerHistorialCliente:', err);
    return res.status(500).json({ error: 'Error interno del servidor.' });
  }
};
...
export const actualizarAbono = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      console.error('Error Supabase al actualizar abono:', error);
      return res.status(500).json({
        error: 'No se pudo actualizar el abono.',
        details: error.message,
      });
    }
    ...
    return res.status(200).json({ message: 'Abono actualizado exitosamente.', pago: data });
  } catch (err) {
