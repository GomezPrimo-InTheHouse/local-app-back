...
import { supabase } from '../../config/supabase.js'; // Usamos el cliente que funciona
import bcrypt from 'bcrypt';
...
export const basicAuth = async (req, res, next) => {
  ...
  try {
    // 3. Consulta a Supabase (Sustituye a pool.query)
    const { data: usuario, error: dbError } = await supabase
      .from('usuario')
      .select('*')
      .eq('email', email)
      .single(); // Nos devuelve un objeto único directamente

    // 4. Validación de existencia de usuario
    if (dbError || !usuario) {
    ...
    // 6. Verificación de presencia de TOTP (la validación del código se hace en el controlador)
    if (!totp) {
      return res.status(400).json({ error: 'Código TOTP requerido' });
    }

    // 7. Preparación de datos para el siguiente paso (login controller)
    req.user = { 
      id: usuario.id, 
      email: usuario.email, 
      rol: usuario.rol // Añadido por si lo necesitas en el token
    };

    next();
  } catch (error) {
    console.error('❌ Error crítico en basicAuth:', error.message);
